...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.title || !body.identifier || !body.content) {
      return NextResponse.json({ success: false, error: "Title, identifier, and content are required" }, { status: 400 });
    }
    
    const db = await getDb();
    const collection = db.collection<MediationClause>(COLLECTIONS.CLAUSES);
    
    const newItem: Omit<MediationClause, "_id"> = {
      ...body,
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    
    const result = await collection.insertOne(newItem as MediationClause);
    ...
    if (userId) {
      AuditRepository.log({
        ...
        details: { title: body.title, identifier: body.identifier }
      });
    }
