...
export default function NegotiationCoursesPage() {
  const { token } = useAuth();
    ...
    if (!course) return (
      <Card className="rounded-3xl border-2 border-dashed border-navy-100 bg-white/50 flex items-center justify-center min-h-[300px]">
        <div className="text-center p-8">
          ...
          <p className="text-navy-950/40 font-medium mb-4">No {type === "foundational" ? "Skilled Negotiator" : "Advanced"} Course</p>
          ...
          <form onSubmit={handleSave} className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-thin">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2"><Label>Course Type</Label><Select value={editingItem?.courseType} onValueChange={(val) => setEditingItem(prev => ({ ...prev!, courseType: val as AcademyCourseType }))}><SelectTrigger><SelectValue placeholder="Select type" /></SelectTrigger><SelectContent><SelectItem value="foundational">Skilled Negotiator Course</SelectItem><SelectItem value="advanced">Advanced Course</SelectItem></SelectContent></Select></div>
              ...
              <div className="space-y-2"><Label>Course Title</Label><Input value={editingItem?.title || ""} onChange={(e) => setEditingItem(prev => ({ ...prev!, title: e.target.value }))} placeholder="e.g. Skilled Negotiator Course" required /></div>
              <div className="space-y-2"><Label>Subtitle</Label><Input value={editingItem?.subtitle || ""} onChange={(e) => setEditingItem(prev => ({ ...prev!, subtitle: e.target.value }))} placeholder="e.g. Master negotiation strategy" /></div>
              <div className="grid grid-cols-2 gap-6">
