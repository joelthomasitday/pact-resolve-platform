  ...
  <FadeInUp className={cn("mb-12 md:mb-20", center ? "flex flex-col items-center text-center" : "")}>
    ...
    <h2 className={cn("text-4xl sm:text-5xl md:text-7xl lg:text-[5rem] font-light tracking-tight mb-8 leading-[0.95]", dark ? "text-white" : "text-navy-950")}>
      {title.split(' ').map((word, i) => (
        <span key={i} className={cn(word.toLowerCase() === 'negotiation' || word.toLowerCase() === 'team' || word.toLowerCase() === 'negotiator' ? "text-gold-500 italic font-medium" : "")}>
          {word}{' '}
        </span>
      ))}
    </h2>
    ...
    <main className="relative min-h-screen w-full overflow-x-hidden bg-white">
      ...
      <section className="py-24 px-6 md:px-12 lg:px-24 bg-white border-t border-navy-100/50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { name: "Faculty Member 1", role: "Negotiation Expert", image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80" },
              { name: "Faculty Member 2", role: "Senior Negotiator", image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80" }
            ].map((faculty, i) => (
